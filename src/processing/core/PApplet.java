...
public class PApplet extends Activity implements PConstants, Runnable {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (DEBUG)
			println("onCreate() happening here: "
	...
	public class SketchSurfaceView2D extends SurfaceView implements
			SketchSurfaceView, SurfaceHolder.Callback {
		...
		public void surfaceChanged(SurfaceHolder holder, int format, int w,
				int h) {
			if (DEBUG) {
				System.out.println("SketchSurfaceView2D.surfaceChanged() " + w
						+ " " + h);
			}
			surfaceChanged = true;
	...
	public class SketchSurfaceView3D extends GLSurfaceView implements
			SketchSurfaceView {
		PGraphicsOpenGL g3;
		SurfaceHolder surfaceHolder;
		...
		public SketchSurfaceView3D(Context context, int wide, int high) {
			...
			if (!supportsGLES2) {
				throw new RuntimeException(
						"P3D: OpenGL ES 2.0 is not supported by this device.");
			}
			...
			if (!depth.equals(DEFAULT_COLOR_DEPTH)) {
				...
				// We use our own context factory and config chooser to set the
				// desired RGBA and depth+stencil
				// bits.
				setEGLContextFactory(g3.pgl.getContextFactory());
				setEGLConfigChooser(g3.pgl.getConfigChooser(val[0], val[1],
		...
		public void surfaceChanged(SurfaceHolder holder, int format, int w,
			...
			if (DEBUG) {
				System.out.println("SketchSurfaceView3D.surfaceChanged() " + w
						+ " " + h);
			}
			surfaceChanged = true;
			// width = w;
	...
	protected void unregisterNoArgs(RegisteredMethods meth, String name,
			Object o) {
		Class<?> c = o.getClass();
		try {
			Method method = c.getMethod(name, new Class[] {});
	...
	public void size(final int iwidth, final int iheight,
		...
		System.out
				.println("See http://wiki.processing.org/w/Android for more information.");
	...
	public PGraphics createGraphics(int iwidth, int iheight, String irenderer) {
		...
		if (irenderer.equals(P2D)) {
			...
			try {
				...
				// class loader, does:
				rendererClass = this.getClass().getClassLoader()
						.loadClass(irenderer);
	...
	public void handleDraw() {
		if (DEBUG) {
			println("inside handleDraw() " + millis() + " changed="
					+ surfaceChanged + " ready=" + surfaceReady + " paused="
					+ paused + " looping=" + looping + " redraw=" + redraw);
		}
		if (surfaceChanged) {
			...
			surfaceReady = true;
			if (DEBUG) {
				println("surfaceChanged true, resized to " + width + "x"
						+ height);
			}
		}
	...
	class PMotionEvent {
		...
		void setPointers(MotionEvent event, int hisIdx) {
			for (int ptIdx = 0; ptIdx < numPointers; ptIdx++) {
				...
				motionPressure[ptIdx] = event.getHistoricalPressure(ptIdx,
						hisIdx); // should this be constrained?
				mouseX[ptIdx] = (int) motionX[ptIdx]; // event.getRawX();
				mouseY[ptIdx] = (int) motionY[ptIdx]; // event.getRawY();
			}
		}
	...
	protected void enqueueMotionEvent(MotionEvent event) {
		synchronized (motionLock) {
			...
			if (motionEventCount + historyCount >= motionEventQueue.length) {
				...
				PMotionEvent[] temp = new PMotionEvent[max(atLeast,
						motionEventCount << 1)];
				if (PApplet.DEBUG) {
					println("motion: " + motionEventCount + " " + historyCount
							+ " " + motionEventQueue.length);
					println("allocating " + temp.length
							+ " entries for motion events");
				}
	...
	protected void handleMotionEvent(PMotionEvent pme) {
		...
		if (pme.action == MotionEvent.ACTION_DOWN
				|| (!mousePressed && numPointers == 1)) {
			...
			if (onePointerGesture) {
				...
				float d0 = PApplet.dist(ppointersX[0], ppointersY[0],
						ppointersX[1], ppointersY[1]);
				float d1 = PApplet.dist(pointersX[0], pointersY[0],
						pointersX[1], pointersY[1]);
			...
			if (onePointerGesture) {
				...
				boolean tap = gestureLength <= MAX_TAP_DISP
						&& gestureTime <= MAX_TAP_DURATION;
				if (tap) {
					mouseClicked();
					tapEvent(downX, downY);
				} else if (MIN_SWIPE_LENGTH <= gestureLength
						&& gestureTime <= MAX_SWIPE_DURATION) {
	...
	public void saveFrame() {
		try {
			...
			System.err
					.println("Can't use saveFrame() when running in a browser, "
	 ...
	 */
	static public final byte[] ICON_IMAGE = { 71, 73, 70, 56, 57, 97, 16, 0,
			16, 0, -77, 0, 0, 0, 0, 0, -1, -1, -1, 12, 12, 13, -15, -15, -14,
			45, 57, 74, 54, 80, 111, 47, 71, 97, 62, 88, 117, 1, 14, 27, 7, 41,
			73, 15, 52, 85, 2, 31, 55, 4, 54, 94, 18, 69, 109, 37, 87, 126, -1,
			-1, -1, 33, -7, 4, 1, 0, 0, 15, 0, 44, 0, 0, 0, 0, 16, 0, 16, 0, 0,
			4, 122, -16, -107, 114, -86, -67, 83, 30, -42, 26, -17, -100, -45,
			56, -57, -108, 48, 40, 122, -90, 104, 67, -91, -51, 32, -53, 77,
			-78, -100, 47, -86, 12, 76, -110, -20, -74, -101, 97, -93, 27, 40,
			20, -65, 65, 48, -111, 99, -20, -112, -117, -123, -47, -105, 24,
			114, -112, 74, 69, 84, 25, 93, 88, -75, 9, 46, 2, 49, 88, -116,
			-67, 7, -19, -83, 60, 38, 3, -34, 2, 66, -95, 27, -98, 13, 4, -17,
			55, 33, 109, 11, 11, -2, -128, 121, 123, 62, 91, 120, -128, 127,
			122, 115, 102, 2, 119, 0, -116, -113, -119, 6, 102, 121, -108,
	...
	public void shape(PShape shape, float x, float y, float z, float c,
			float d, float e) {
